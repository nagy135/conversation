...
Open http://localhost:5173. Allow microphone access and press play. Press the same button to stop. The visible transcript remains until the next conversation. Only the cohesive memory summary is saved in this browser’s localStorage and carried into new sessions. New transcript fragments stay in a temporary in-memory buffer. Every 30 seconds when text is pending, and when a session ends, the browser asynchronously asks `gpt-5.6-terra` to merge the previous summary with only the new buffer. Successful runs save the replacement summary and discard the consumed fragments; text arriving during a run stays buffered for the next run. Large buffers are processed in bounded batches. Voice never waits for summarization. Failures retry while the page stays open. Closing or reloading the page loses unsummarized text. Previously stored raw conversations are removed when this version loads. Open **Memory** to view the summary, update it, or clear it after stopping. Memory is specific to this browser profile and origin; concurrent tabs do not merge memory. Clearing browser site data also removes it. If browser storage is unavailable, memory only lasts for this page and the Memory panel shows a warning.
